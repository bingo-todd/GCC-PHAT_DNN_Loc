...
def gcc_window_test():
    ...
    ##
    win_funcs = {'rectangle':np.ones,'hanning':np.hanning,'hamming':np.hamming}
    ...
    plt.tight_layout()
    plot_tools.savefig(fig,name='gcc_phat_diff_window.png')
    ...
    for i,func_name in enumerate(win_funcs.keys()):
        ...
        ax[i].set_title(func_name)
    plt.tight_layout()
    plot_tools.savefig(fig,name='gcc_phat_diff_window_range_limited.png')
